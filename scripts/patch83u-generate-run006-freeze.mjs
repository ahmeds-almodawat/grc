...
import {
  ...
  RUN006_EVIDENCE_DIRECTORY,
  RUN006_EVIDENCE_SCHEMA_PATH,
  RUN006_OPERATOR_CONFIRMATION_CONTRACT_ID,
  RUN006_OUTPUT_PATH_PATTERN,
  RUN006_PROOF_CONTRACT_ID,
  RUN006_PROOF_CONTRACT_PATH,
  RUN006_PROOF_CONTRACT_VERSION,
  RUN006_TRACEABILITY_MARKDOWN_PATH,
  RUN006_TRACEABILITY_PATH,
  RUN006_TRACEABILITY_VERSION,
  RUN_NUMBER,
...
import {
  ...
  validateRun006ProofContract,
  validateRun006Traceability,
  verifyRun006ContractArtifacts,
} from './patch83u-run006-contract-audit.mjs';
...
const V6_MARKDOWN =
  'release/patch83u/patch83u-staging-reset-execution-freeze-v6-20260720.md';
const V6_SCHEMA =
  'release/patch83u/patch83u-staging-reset-execution-freeze-schema-v6.json';
const EDGE_PROVENANCE =
  'release/patch83u/patch83u-staging-edge-v5-provenance-20260719.json';
const RUN006_RESERVATION =
  'release/patch83u/reset-proof-run-006/run006-reservation.json';
const EXPECTED_V5_SHA256 =
  '52013d69ca30ea38a0765832c28888565db280adcedb575ed8646fb1429c8e0b';
const EXPECTED_V5_BYTES = 32091;
const EXPECTED_PRIOR_AGGREGATE =
  'c8cc9b2859e588f0b9032ec9e8dc930aae28807c0b53cff342f247cd0094c790';
const EXPECTED_PRIOR_BYTES = 257610;
const EXPECTED_PRIOR_COUNT = 35;
...
const V5_SOURCE_EXCLUSIONS = Object.freeze(new Set([
  'release/patch83u/patch83u-staging-checkpoint-file-schema.json',
  'release/patch83u/patch83u-staging-reset-execution-freeze-schema-v5.json',
  'release/patch83u/patch83u-staging-reset-harness-schema-v2.json',
  'release/patch83u/patch83u-staging-reset-harness-schema-v3.json',
  'release/patch83u/patch83u-staging-reset-harness-schema-v4.json',
...
const RUN006_SOURCE_ADDITIONS = Object.freeze([
  V6_SCHEMA,
  RUN006_CHECKPOINT_SCHEMA_PATH,
  RUN006_EVIDENCE_SCHEMA_PATH,
  RUN006_PROOF_CONTRACT_PATH,
  RUN006_TRACEABILITY_PATH,
  RUN006_TRACEABILITY_MARKDOWN_PATH,
  'release/patch83u/reset-proof-run-006/README.md',
  'release/patch83u/reset-proof-run-006/checkpoints/README.md',
  RUN006_RESERVATION,
  'scripts/patch83u-generate-run006-freeze.mjs',
  'scripts/patch83u-run006-contract-audit.mjs',
  'tests/unit/patch83uRun006CompletionEvidenceContract.test.ts',
]);
...
async function safeFile(path) {
  if (isAbsolute(path) || path.includes('\\')) {
    throw new Error(`PATCH83U_RUN006_FREEZE_PATH_REFUSED:${path}`);
  }
  ...
  if (!stat.isFile() || stat.isSymbolicLink()) {
    throw new Error(`PATCH83U_RUN006_FREEZE_FILE_REFUSED:${path}`);
  }
...
async function toolchain() {
  ...
  if (typeof playwright !== 'string' || playwright === '') {
    throw new Error('PATCH83U_RUN006_PLAYWRIGHT_VERSION_UNAVAILABLE');
  }
  return {
...
function freezeMarkdown(freeze, freezeJsonSha256) {
  return `# Patch 83U staging reset execution freeze V6
...
async function buildFreeze(state) {
  const v5Bytes = await readFile(resolve(ROOT, V5_JSON));
  if (
    v5Bytes.length !== EXPECTED_V5_BYTES
    || sha256(v5Bytes) !== EXPECTED_V5_SHA256
  ) {
    throw new Error('PATCH83U_RUN006_V5_SUPERSEDED_FREEZE_DRIFT');
  }
  const v5 = JSON.parse(v5Bytes.toString('utf8'));
  const sourcePaths = [
    ...v5.frozen_source.files.map((entry) => entry.path)
      .filter((path) => !V5_SOURCE_EXCLUSIONS.has(path)),
    ...RUN006_SOURCE_ADDITIONS,
  ].sort();
  if (new Set(sourcePaths).size !== sourcePaths.length) {
    throw new Error('PATCH83U_RUN006_SOURCE_INVENTORY_DUPLICATE');
  }
  const frozenFiles = [];
  for (const path of sourcePaths) {
    ...
    if (!['clean', ' M', '??'].includes(gitStatus)) {
      throw new Error(`PATCH83U_RUN006_SOURCE_GIT_STATUS_REFUSED:${path}`);
    }
    frozenFiles.push({
      ...file,
  ...
  const priorPaths = [
    ...v5.prior_evidence_integrity.files.map((entry) => entry.path),
    ...PRIOR_EVIDENCE_ADDITIONS,
  ].sort();
  if (new Set(priorPaths).size !== priorPaths.length) {
    throw new Error('PATCH83U_RUN006_PRIOR_EVIDENCE_DUPLICATE');
  }
  const priorFiles = [];
  ...
  if (
    ...
    throw new Error('PATCH83U_RUN006_PRIOR_EVIDENCE_BASELINE_MISMATCH');
  }
  ...
  const [
    ...
    readFile(resolve(ROOT, V6_SCHEMA)),
    readFile(resolve(ROOT, EDGE_PROVENANCE)),
    readFile(resolve(ROOT, RUN006_EVIDENCE_SCHEMA_PATH)),
    readFile(resolve(ROOT, RUN006_CHECKPOINT_SCHEMA_PATH)),
    readFile(resolve(ROOT, RUN006_PROOF_CONTRACT_PATH)),
    readFile(resolve(ROOT, RUN006_TRACEABILITY_PATH)),
    readFile(resolve(ROOT, RUN006_TRACEABILITY_MARKDOWN_PATH)),
    toolchain(),
  ]);
  ...
  const traceabilityValidation = validateRun006Traceability(
    traceability,
    proofContract,
  );
  if (
    ...
    throw new Error('PATCH83U_RUN006_PROOF_TRACEABILITY_INCOMPLETE');
  }
  ...
  const freeze = {
    $schema: './patch83u-staging-reset-execution-freeze-schema-v6.json',
    ...
    supersedes: {
      path: V5_JSON,
      sha256: EXPECTED_V5_SHA256,
      bytes: EXPECTED_V5_BYTES,
    ...
    operator_confirmation_contract: {
      contract_id: RUN006_OPERATOR_CONFIRMATION_CONTRACT_ID,
  ...
  if (!validation.valid) {
    throw new Error(
      `PATCH83U_RUN006_FREEZE_SCHEMA_INVALID:${validation.errors[0] ?? 'UNKNOWN'}`,
    );
  }
  assertExecutionFreezeSemanticContract(freeze);
  const contractAudit = await verifyRun006ContractArtifacts({
    freeze,
    repositoryRoot: ROOT,
  });
  if (
    ...
    throw new Error('PATCH83U_RUN006_PRE_CREDENTIAL_STATIC_AUDIT_INCOMPLETE');
  }
  if (
    freeze.final_session_contract.expected_session_count
      !== RUN006_EXPECTED_SESSION_COUNT
    ...
    throw new Error('PATCH83U_RUN006_FINAL_SESSION_CONTRACT_DRIFT');
  }
  return freeze;
}
...
async function main(argv = process.argv.slice(2)) {
  if (argv.length !== 0) {
    throw new Error('PATCH83U_RUN006_FREEZE_GENERATOR_ARGUMENTS_REFUSED');
  }
  const jsonPath = resolve(ROOT, V6_JSON);
  const markdownPath = resolve(ROOT, V6_MARKDOWN);
  const createdPaths = [];
  let jsonHandle;
  let markdownHandle;
  try {
    ...
    if (
      ...
      throw new Error('PATCH83U_RUN006_REPOSITORY_GATE_FAILED');
    }
    await verifyRunOutputAbsence('005');
    await verifyRunOutputAbsence('006');
    ...
    process.stdout.write(
      `V6 aggregate SHA-256: ${freeze.frozen_source.aggregate_sha256}\n`
      + `V6 freeze JSON SHA-256: ${freezeJsonSha256}\n`,
    );
  } catch (error) {
    await jsonHandle?.close();
    await markdownHandle?.close();
...
main().catch((error) => {
  process.stderr.write(
    `${error instanceof Error ? error.message : 'PATCH83U_RUN006_FREEZE_FAILED'}\n`,
  );
  process.exitCode = 1;
});
