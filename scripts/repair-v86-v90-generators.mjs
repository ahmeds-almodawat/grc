...
function writeScript(filePath, config) {
...
for (const config of configs) {
  writeScript(config.file, config);
}

const reviewScript = `import fs from 'node:fs';
import path from 'node:path';
...
const rows = sections.map((section) => '| ' + section[0] + ' | ' + section[1] + ' | technical_ready_pending_human_approval | No | ' + section[2] + ' |').join('\\n');

const md = \`# v8.6 to v9.0 Pilot Execution Evidence Suite Review Pack
...
fs.writeFileSync('scripts/v86-v90-generate-review-pack.mjs', reviewScript);

console.log('v8.6-v9.0 generator scripts repaired.');
