...
const reportPath = path.join(releaseDir, 'v170-enterprise-risk-report.md');
...
const proof = {
  ...
  recommendation:
    failed.length === 0
      ? 'Ready for controlled enterprise risk management pilot validation.'
      : 'Do not commit v17 until failed proof checks are corrected.',
};

fs.mkdirSync(releaseDir, { recursive: true });
fs.writeFileSync(path.join(releaseDir, 'v170-final-proof.json'), `${JSON.stringify(proof, null, 2)}\n`);

console.log('v17.0 final proof complete.');
