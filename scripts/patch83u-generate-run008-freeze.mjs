...
import {
  ...
  RUN008_EVIDENCE_DIRECTORY,
  RUN008_EVIDENCE_SCHEMA_PATH,
  RUN008_OPERATOR_CONFIRMATION_CONTRACT_ID,
  RUN008_OUTPUT_PATH_PATTERN,
  RUN008_PROOF_CONTRACT_ID,
  RUN008_PROOF_CONTRACT_PATH,
  RUN008_PROOF_CONTRACT_VERSION,
  RUN008_TRACEABILITY_MARKDOWN_PATH,
  RUN008_TRACEABILITY_PATH,
  RUN008_TRACEABILITY_VERSION,
  RUN_NUMBER,
...
} from './patch83u-run008-contract-audit.mjs';
...
const RUN008_RESERVATION =
  'release/patch83u/reset-proof-run-008/run008-reservation.json';
const EXPECTED_PRIOR_FREEZE_SHA256 =
  '5925dddf34342fb85be739d2d50149b116ed7097712d45b83ea5e6a3ca8806d4';
const EXPECTED_PRIOR_FREEZE_BYTES = 40829;
...
const RUN008_SOURCE_ADDITIONS = Object.freeze([
  V8_SCHEMA,
  RUN008_EVIDENCE_SCHEMA_PATH,
  RUN008_PROOF_CONTRACT_PATH,
  RUN008_TRACEABILITY_PATH,
  RUN008_TRACEABILITY_MARKDOWN_PATH,
  'release/patch83u/reset-proof-run-008/README.md',
  'release/patch83u/reset-proof-run-008/checkpoints/README.md',
  RUN008_RESERVATION,
  'scripts/patch83u-generate-run008-freeze.mjs',
  'scripts/patch83u-run008-contract-audit.mjs',
  'scripts/patch83u-run008-dpapi-credentials.mjs',
  'tests/unit/patch83uRun008CompletionEvidenceContract.test.ts',
  'tests/unit/patch83uRun008DpapiCredentials.test.ts',
...
async function safeFile(path) {
  if (isAbsolute(path) || path.includes('\\')) {
    throw new Error(`PATCH83U_RUN008_FREEZE_PATH_REFUSED:${path}`);
  }
  ...
  if (!stat.isFile() || stat.isSymbolicLink()) {
    throw new Error(`PATCH83U_RUN008_FREEZE_FILE_REFUSED:${path}`);
  }
...
async function toolchain() {
  ...
  if (typeof playwright !== 'string' || playwright === '') {
    throw new Error('PATCH83U_RUN008_PLAYWRIGHT_VERSION_UNAVAILABLE');
  }
  return {
...
async function buildFreeze(state) {
  ...
  if (
    ...
    throw new Error('PATCH83U_RUN008_PRIOR_FREEZE_DRIFT');
  }
  ...
  const sourcePaths = [
    ...
    ...RUN008_SOURCE_ADDITIONS,
  ].sort();
  if (new Set(sourcePaths).size !== sourcePaths.length) {
    throw new Error('PATCH83U_RUN008_SOURCE_INVENTORY_DUPLICATE');
  }
  const frozenFiles = [];
  for (const path of sourcePaths) {
    ...
    if (!['clean', ' M', '??'].includes(gitStatus)) {
      throw new Error(`PATCH83U_RUN008_SOURCE_GIT_STATUS_REFUSED:${path}`);
    }
    frozenFiles.push({
      ...file,
  ...
  if (new Set(priorPaths).size !== priorPaths.length) {
    throw new Error('PATCH83U_RUN008_PRIOR_EVIDENCE_DUPLICATE');
  }
  const priorFiles = [];
  ...
  const [
    ...
    readFile(resolve(ROOT, V8_SCHEMA)),
    readFile(resolve(ROOT, EDGE_PROVENANCE)),
    readFile(resolve(ROOT, RUN008_EVIDENCE_SCHEMA_PATH)),
    readFile(resolve(ROOT, RUN008_CHECKPOINT_SCHEMA_PATH)),
    readFile(resolve(ROOT, RUN008_PROOF_CONTRACT_PATH)),
    readFile(resolve(ROOT, RUN008_TRACEABILITY_PATH)),
    readFile(resolve(ROOT, RUN008_TRACEABILITY_MARKDOWN_PATH)),
    toolchain(),
  ]);
  ...
  const traceabilityValidation = validateRun008Traceability(
    traceability,
    proofContract,
  );
  if (
    ...
    throw new Error('PATCH83U_RUN008_PROOF_TRACEABILITY_INCOMPLETE');
  }
  ...
  if (!validation.valid) {
    throw new Error(
      `PATCH83U_RUN008_FREEZE_SCHEMA_INVALID:${validation.errors[0] ?? 'UNKNOWN'}`,
    );
  }
  assertExecutionFreezeSemanticContract(freeze);
  const contractAudit = await verifyRun008ContractArtifacts({
    freeze,
    repositoryRoot: ROOT,
  });
  if (
    ...
    throw new Error('PATCH83U_RUN008_PRE_CREDENTIAL_STATIC_AUDIT_INCOMPLETE');
  }
  if (
    freeze.final_session_contract.expected_session_count
      !== RUN008_EXPECTED_SESSION_COUNT
    ...
    throw new Error('PATCH83U_RUN008_FINAL_SESSION_CONTRACT_DRIFT');
  }
  return freeze;
}
...
async function main(argv = process.argv.slice(2)) {
  if (argv.length !== 0) {
    throw new Error('PATCH83U_RUN008_FREEZE_GENERATOR_ARGUMENTS_REFUSED');
  }
  const jsonPath = resolve(ROOT, V8_JSON);
  const markdownPath = resolve(ROOT, V8_MARKDOWN);
  const createdPaths = [];
  let jsonHandle;
  let markdownHandle;
  try {
    ...
    if (
      ...
      throw new Error('PATCH83U_RUN008_REPOSITORY_GATE_FAILED');
    }
    await verifyRunOutputAbsence('008');
    ...
    process.stdout.write(
      `V8 aggregate SHA-256: ${freeze.frozen_source.aggregate_sha256}\n`
      + `V8 freeze JSON SHA-256: ${freezeJsonSha256}\n`,
    );
  } catch (error) {
    await jsonHandle?.close();
    await markdownHandle?.close();
...
main().catch((error) => {
  process.stderr.write(
    `${error instanceof Error ? error.message : 'PATCH83U_RUN008_FREEZE_FAILED'}\n`,
  );
  process.exitCode = 1;
});
