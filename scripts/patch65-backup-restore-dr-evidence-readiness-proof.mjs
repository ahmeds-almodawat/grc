...
const reportDir = path.join(root, 'release', 'patch65');
const reportPath = path.join(reportDir, 'patch65-backup-restore-dr-evidence-readiness-proof.json');
...
const patch65Migrations = fs.existsSync(migrationDir)
  ? fs.readdirSync(migrationDir).filter((name) => /patch65|121_patch65/i.test(name))
  : [];
...
const checks = [
  { name: 'no Patch 65 migration exists', passed: patch65Migrations.length === 0, findings: patch65Migrations },
  ...
  { name: 'backup/restore/DR helper exists', passed: apiSource.includes('getBackupRestoreDrEvidenceReadiness') && apiSource.includes('getBackupRestoreDrGapSummary') },
  { name: 'backup and restore evidence wording exists', passed: pageSource.includes('Backup and restore evidence') },
  { name: 'DR evidence wording exists', passed: pageSource.includes('DR evidence') || pageSource.includes('DR/recovery evidence') },
  { name: 'backup evidence required wording exists', passed: apiSource.includes('Backup evidence required') && pageSource.includes('Backup evidence required') },
  { name: 'restore test evidence required wording exists', passed: apiSource.includes('Restore test evidence required') && pageSource.includes('Restore test evidence required') },
  { name: 'DR evidence required wording exists', passed: apiSource.includes('DR evidence required') && pageSource.includes('DR evidence required') },
  ...
  { name: 'source workflow destination wording remains', passed: pageSource.includes('Source workflow destination') && pageSource.includes('Manage recovery evidence in Production Readiness Center') },
  { name: 'recovery caveat wording exists', passed: apiSource.includes('Recovery readiness depends on recorded source evidence.') && pageSource.includes('Recovery readiness depends on recorded source evidence') },
  ...
  { name: 'operator console mentions backup, restore, and DR evidence readiness', passed: operatorSource.includes('backup, restore, and DR evidence readiness') },
  ...
  { name: 'restore-noise covers Patch 64 generated proof JSON', passed: restoreSource.includes('release/patch64/patch64-policy-sop-attestation-evidence-readiness-proof.json') },
  { name: 'package patch65:proof exists', passed: scripts['patch65:proof'] === 'node scripts/patch65-backup-restore-dr-evidence-readiness-proof.mjs' },
  { name: 'package patch65:all exists', passed: scripts['patch65:all'] === 'npm run validate:build && npm run patch65:proof' },
  ...
  { name: 'status doc mentions Patch 65', passed: statusSource.includes('Patch 65') && statusSource.includes('backup, restore, and DR evidence readiness') },
