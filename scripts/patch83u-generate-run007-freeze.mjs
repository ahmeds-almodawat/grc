...
import {
  ...
  validateRun007ProofContract,
  validateRun007Traceability,
  verifyRun007ContractArtifacts,
} from './patch83u-run007-contract-audit.mjs';
...
const V7_JSON =
  'release/patch83u/patch83u-staging-reset-execution-freeze-v7-20260721.json';
const V7_MARKDOWN =
  'release/patch83u/patch83u-staging-reset-execution-freeze-v7-20260721.md';
const V7_SCHEMA =
  'release/patch83u/patch83u-staging-reset-execution-freeze-schema-v7.json';
const EDGE_PROVENANCE =
  'release/patch83u/patch83u-staging-edge-v5-provenance-20260719.json';
const RUN007_RESERVATION =
  'release/patch83u/reset-proof-run-007/run007-reservation.json';
const EXPECTED_PRIOR_FREEZE_SHA256 =
  '741a610233a8175e75018587c297cce05809f9259957f9f7c9028301495a29d9';
const EXPECTED_PRIOR_FREEZE_BYTES = 36015;
const EXPECTED_PRIOR_AGGREGATE =
  '39854df2a5b362eaaebf7375a090e2d7e329ebc714089991c1df609fc5ae4867';
const EXPECTED_PRIOR_BYTES = 355203;
const EXPECTED_PRIOR_COUNT = 42;
...
const RUN007_SOURCE_ADDITIONS = Object.freeze([
  V7_SCHEMA,
  RUN007_CHECKPOINT_SCHEMA_PATH,
  RUN007_EVIDENCE_SCHEMA_PATH,
  RUN007_PROOF_CONTRACT_PATH,
  RUN007_TRACEABILITY_PATH,
  RUN007_TRACEABILITY_MARKDOWN_PATH,
  'release/patch83u/reset-proof-run-007/README.md',
  'release/patch83u/reset-proof-run-007/checkpoints/README.md',
  RUN007_RESERVATION,
  'scripts/patch83u-generate-run007-freeze.mjs',
  'scripts/patch83u-run007-contract-audit.mjs',
  'tests/unit/patch83uRun007CompletionEvidenceContract.test.ts',
]);
...
async function safeFile(path) {
  if (isAbsolute(path) || path.includes('\\')) {
    throw new Error(`PATCH83U_RUN007_FREEZE_PATH_REFUSED:${path}`);
  }
  ...
  if (!stat.isFile() || stat.isSymbolicLink()) {
    throw new Error(`PATCH83U_RUN007_FREEZE_FILE_REFUSED:${path}`);
  }
...
async function toolchain() {
  ...
  if (typeof playwright !== 'string' || playwright === '') {
    throw new Error('PATCH83U_RUN007_PLAYWRIGHT_VERSION_UNAVAILABLE');
  }
  return {
...
function freezeMarkdown(freeze, freezeJsonSha256) {
  return `# Patch 83U staging reset execution freeze V7
...
async function buildFreeze(state) {
  ...
  if (
    ...
    throw new Error('PATCH83U_RUN007_PRIOR_FREEZE_DRIFT');
  }
  ...
  const sourcePaths = [
    ...
    ...RUN007_SOURCE_ADDITIONS,
  ].sort();
  if (new Set(sourcePaths).size !== sourcePaths.length) {
    throw new Error('PATCH83U_RUN007_SOURCE_INVENTORY_DUPLICATE');
  }
  const frozenFiles = [];
  for (const path of sourcePaths) {
    ...
    if (!['clean', ' M', '??'].includes(gitStatus)) {
      throw new Error(`PATCH83U_RUN007_SOURCE_GIT_STATUS_REFUSED:${path}`);
    }
    frozenFiles.push({
      ...file,
  ...
  if (new Set(priorPaths).size !== priorPaths.length) {
    throw new Error('PATCH83U_RUN007_PRIOR_EVIDENCE_DUPLICATE');
  }
  const priorFiles = [];
  ...
  if (
    ...
    throw new Error(
      'PATCH83U_RUN007_PRIOR_EVIDENCE_BASELINE_MISMATCH:'
      + `${priorFiles.length}:${priorBytes}:${manifestAggregate(priorFiles)}`,
    );
  }
  ...
  const [
    ...
    readFile(resolve(ROOT, V7_SCHEMA)),
    readFile(resolve(ROOT, EDGE_PROVENANCE)),
    readFile(resolve(ROOT, RUN007_EVIDENCE_SCHEMA_PATH)),
    readFile(resolve(ROOT, RUN007_CHECKPOINT_SCHEMA_PATH)),
    readFile(resolve(ROOT, RUN007_PROOF_CONTRACT_PATH)),
    readFile(resolve(ROOT, RUN007_TRACEABILITY_PATH)),
    readFile(resolve(ROOT, RUN007_TRACEABILITY_MARKDOWN_PATH)),
    toolchain(),
  ]);
  ...
  const traceabilityValidation = validateRun007Traceability(
    traceability,
    proofContract,
  );
  if (
    ...
    throw new Error('PATCH83U_RUN007_PROOF_TRACEABILITY_INCOMPLETE');
  }
  ...
  const freeze = {
    $schema: './patch83u-staging-reset-execution-freeze-schema-v7.json',
    ...
    proof_contract: {
      path: RUN007_PROOF_CONTRACT_PATH,
      sha256: sha256(proofContractBytes),
      bytes: proofContractBytes.length,
      schema_version: RUN007_PROOF_CONTRACT_VERSION,
      contract_id: RUN007_PROOF_CONTRACT_ID,
      requirement_count: proofValidation.requirement_count,
    },
    checkpoint_schema: {
      path: RUN007_CHECKPOINT_SCHEMA_PATH,
  ...
  if (!validation.valid) {
    throw new Error(
      `PATCH83U_RUN007_FREEZE_SCHEMA_INVALID:${validation.errors[0] ?? 'UNKNOWN'}`,
    );
  }
  assertExecutionFreezeSemanticContract(freeze);
  const contractAudit = await verifyRun007ContractArtifacts({
    freeze,
    repositoryRoot: ROOT,
  });
  if (
    ...
    throw new Error('PATCH83U_RUN007_PRE_CREDENTIAL_STATIC_AUDIT_INCOMPLETE');
  }
  if (
    freeze.final_session_contract.expected_session_count
      !== RUN007_EXPECTED_SESSION_COUNT
    ...
    throw new Error('PATCH83U_RUN007_FINAL_SESSION_CONTRACT_DRIFT');
  }
  return freeze;
}
...
async function main(argv = process.argv.slice(2)) {
  if (argv.length !== 0) {
    throw new Error('PATCH83U_RUN007_FREEZE_GENERATOR_ARGUMENTS_REFUSED');
  }
  const jsonPath = resolve(ROOT, V7_JSON);
  const markdownPath = resolve(ROOT, V7_MARKDOWN);
  const createdPaths = [];
  let jsonHandle;
  let markdownHandle;
  try {
    ...
    if (
      ...
      throw new Error('PATCH83U_RUN007_REPOSITORY_GATE_FAILED');
    }
    await verifyRunOutputAbsence('005');
    await verifyRunOutputAbsence('006');
    await verifyRunOutputAbsence('007');
    ...
    process.stdout.write(
      `V7 aggregate SHA-256: ${freeze.frozen_source.aggregate_sha256}\n`
      + `V7 freeze JSON SHA-256: ${freezeJsonSha256}\n`,
    );
  } catch (error) {
    await jsonHandle?.close();
    await markdownHandle?.close();
...
main().catch((error) => {
  process.stderr.write(
    `${error instanceof Error ? error.message : 'PATCH83U_RUN007_FREEZE_FAILED'}\n`,
  );
  process.exitCode = 1;
});
