...
- Reviewed restricted migration 176–197 SECURITY DEFINER routines: 83
...
| `public.patch83u_credential_access_allowed()` | target_migrations_171_205 | no | no | yes | allowed | Credential version, state, email, and session freshness decision used by restrictive RLS. |
| `public.patch83u_profile_update_allowed(p_target_user_id uuid, p_target_organization_id uuid)` | target_migrations_171_205 | no | no | yes | allowed | Same-organization credential-active profile update decision used by restrictive RLS. |
| `public.patch83u_user_role_mutation_allowed(p_target_user_id uuid, p_role public.app_role, p_scope public.access_scope, p_role_organization_id uuid, p_division_id uuid, p_department_id uuid, p_unit_id uuid)` | target_migrations_171_205 | no | no | yes | allowed | Credential-active canonical role/scope mutation decision used by restrictive RLS. |
...
| `public.fn_immutability_sop_training_target_scope()` | migration205_service_role_acl_review | yes | service_role_only | `supabase/migrations/205_governed_sop_training_and_competency_lifecycle.sql:178` |
| `public.fn_validate_sop_training_assignment()` | migration205_service_role_acl_review | yes | service_role_only | `supabase/migrations/205_governed_sop_training_and_competency_lifecycle.sql:211` |
| `public.fn_validate_sop_training_target_scope()` | migration205_service_role_acl_review | yes | service_role_only | `supabase/migrations/205_governed_sop_training_and_competency_lifecycle.sql:108` |
...
| `public.publish_sop_training_obligations(p_actor_id uuid, p_version_id uuid)` | migration205_service_role_acl_review | yes | service_role_only | `supabase/migrations/205_governed_sop_training_and_competency_lifecycle.sql:593` |
| `public.reconcile_sop_training_population(p_actor_id uuid, p_version_id uuid)` | migration205_service_role_acl_review | yes | service_role_only | `supabase/migrations/205_governed_sop_training_and_competency_lifecycle.sql:806` |
| `public.record_competency_assessment(p_assignment_id uuid, p_user_id uuid, p_competency_area text, p_result text, p_score numeric, p_evidence_id uuid, p_notes text, p_actor_id uuid)` | migration205_service_role_acl_review | yes | service_role_only | `supabase/migrations/205_governed_sop_training_and_competency_lifecycle.sql:985` |
...
| `public.start_governed_document_revision(p_actor_id uuid, p_source_version_id uuid, p_revision_type text default 'minor', p_revision_reason text default null)` | migration205_service_role_acl_review | yes | service_role_only | `supabase/migrations/205_governed_sop_training_and_competency_lifecycle.sql:372` |
