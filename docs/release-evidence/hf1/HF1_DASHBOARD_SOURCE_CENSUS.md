...
| Recent governed activity | `getRecentGovernedActivity` -> `v_recent_governed_activity` | Latest 12 | Security-invoker view; authenticated SELECT; anonymous denied; source-table RLS | View existed and returned 0 rows, but UI was hardcoded unavailable | F | Trusted view is wired; zero renders a scoped empty state and failures render unavailable |
