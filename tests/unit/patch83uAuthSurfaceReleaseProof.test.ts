...
describe('Patch 83U authenticated surface release proof', () => {
  ...
  it('fails closed when future migration 215 introduces an unaudited SECURITY DEFINER routine', () => {
    const report = analyzePatch83uAuthSurface({
      migrationFiles: [{
        path: 'supabase/migrations/215_future_fixture.sql',
        text: `
  ...
  it('fails closed when future migration 215 makes a SECURITY DEFINER routine browser executable', () => {
    const report = analyzePatch83uAuthSurface({
      migrationFiles: [{
        path: 'supabase/migrations/215_browser_fixture.sql',
        text: `
  ...
  it('proves the target repository leaves no direct RPC exception and credential-locks global search through RLS', () => {
    ...
    expect(report.summary.reviewed_patch83u_migration_ceiling).toBe(214);
    expect(report.summary.reviewed_restricted_security_definer_count).toBe(123);
    expect(report.acl_reachable_security_definer_rpcs.reviewed_restricted_security_definers)
      .toEqual(expect.arrayContaining([
        expect.objectContaining({
