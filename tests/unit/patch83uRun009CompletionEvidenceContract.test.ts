...
import {
  ...
  assertCompletedExecutionEvidence,
...
function run009OperatorConfirmationContract() {
  return {
...
function resetEnvelope(requestId = 'patch83u-run009-request-001') {
  return {
...
describe('Patch 83U Run 009 completion evidence contract', () => {
  ...
  it('freeze-binds Run 009 confirmation and final 1/1 contracts together', () => {
    expect(assertRun007ConfirmationAndFinalSessionContracts({
      operatorConfirmationContract: run009OperatorConfirmationContract(),
    ...
    for (const drift of [
      {
        operatorConfirmationContract: {
          ...run009OperatorConfirmationContract(),
  ...
  it('proves the reset request correlation is generated at submission and remains one stable hash', async () => {
    ...
    expect(() => premature.observeRequestId('patch83u-run009-request-001')).toThrow(
