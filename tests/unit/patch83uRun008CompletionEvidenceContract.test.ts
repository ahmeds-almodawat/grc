...
import {
  RUN008_CONFIRMATION_CONTRACT_ID,
} from '../../scripts/patch83u-run008-contract-audit.mjs';

const evidenceSchema = JSON.parse(
  readFileSync(
    resolve('release/patch83u/patch83u-staging-reset-harness-schema-v7.json'),
    'utf8',
  ),
);
...
function run008OperatorConfirmationContract() {
  return {
    contract_id: RUN008_CONFIRMATION_CONTRACT_ID,
    run_number: 8,
...
function currentOperatorConfirmationContract() {
  return {
    ...run008OperatorConfirmationContract(),
...
function assertHistoricalConfirmationAndFinalSessionContracts(input: any) {
  ...
  if (
    ...
    || confirmation?.run_number !== 8
    || confirmation?.exact_phrase !== 'EXECUTE RUN 008 RESET NOW'
    ...
    throw new Error('PATCH83U_RUN008_CONFIRMATION_AND_FINAL_SESSION_CONTRACTS_FAILED');
  }
  return input;
}
...
function resetEnvelope(requestId = 'patch83u-run008-request-001') {
  return {
...
describe('Patch 83U Run 008 completion evidence contract', () => {
  ...
  it('freeze-binds Run 008 confirmation and final 1/1 contracts together', () => {
    expect(assertHistoricalConfirmationAndFinalSessionContracts({
      operatorConfirmationContract: run008OperatorConfirmationContract(),
  ...
  it('proves the reset request correlation is generated at submission and remains one stable hash', async () => {
    ...
    expect(() => premature.observeRequestId('patch83u-run008-request-001')).toThrow(
