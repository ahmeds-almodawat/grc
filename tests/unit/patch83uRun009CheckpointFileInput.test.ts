...
describe('Patch 83U SQL Editor checkpoint-file input', () => {
  ...
  it('uses six exact deterministic Run 007 filenames', () => {
    ...
    expect(gitignoreSource).not.toContain(
      '/release/patch83u/reset-proof-run-009/checkpoints/*.json',
    );
    for (const fileName of Object.values(SQL_EDITOR_CHECKPOINT_FILE_NAMES)) {
      expect(gitignoreSource).toContain(
        `/release/patch83u/reset-proof-run-009/checkpoints/${fileName}`,
      );
    }
  });
  ...
  it('binds output to a new, non-checkpoint Run 007 attempt file', async () => {
    const valid =
      'release/patch83u/reset-proof-run-009/'
      + 'patch83u-staging-reset-final-results-attempt-999.json';
    ...
    for (const refused of [
      ...
      'release/patch83u/reset-proof-run-009/checkpoints/02-immediately-before-reset.json',
      'release/patch83u/reset-proof-run-009/../result.json',
