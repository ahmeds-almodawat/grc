...
import {
  ...
  SecretValue,
  assertAfterPasswordChangeAggregate as assertCurrentAfterPasswordChangeAggregate,
  assertAdminContextReadinessProof,
  assertBeforeRequiredPasswordChangeAggregate as assertCurrentBeforeRequiredPasswordChangeAggregate,
  ...
  assertFinalFreshLoginAggregate as assertCurrentFinalFreshLoginAggregate,
  assertFrontendOriginExactAggregate,
  assertFreshPermanentLoginProof,
  assertInitialEmployeeStateAggregate as assertCurrentInitialEmployeeStateAggregate,
  assertNetworkConsoleSafeAggregate,
  assertNonpersistentBrowserContext,
  assertOneShotResetAggregate,
  assertPasswordChangeCompletionProof as assertCurrentPasswordChangeCompletionProof,
  assertPostResetSuccessAggregate as assertCurrentPostResetSuccessAggregate,
  assertPreResetCheckpointAggregate as assertCurrentPreResetCheckpointAggregate,
  assertProtectedStateFailClosedAggregate,
  assertRuntimeAndContractsExactAggregate as assertCurrentRuntimeAndContractsExactAggregate,
  ...
  assertExactResetConfirmationsProof as assertCurrentExactResetConfirmationsProof,
...
import {
  RUN006_CONFIRMATION_CONTRACT_ID,
} from '../../scripts/patch83u-run006-contract-audit.mjs';

const evidenceSchema = JSON.parse(
  readFileSync(
    resolve('release/patch83u/patch83u-staging-reset-harness-schema-v5.json'),
    'utf8',
  ),
);
const checkpointSchema = JSON.parse(
  readFileSync(
    resolve('release/patch83u/patch83u-staging-checkpoint-file-schema-v2.json'),
    'utf8',
  ),
);
...
function run006OperatorConfirmationContract() {
  return {
    contract_id: RUN006_CONFIRMATION_CONTRACT_ID,
    run_number: 6,
    exact_phrase: 'EXECUTE RUN 006 RESET NOW',
    case_sensitive: true,
...
function currentOperatorConfirmationContract() {
  return {
    ...run006OperatorConfirmationContract(),
...
const historicalCredentialVersionMap = new Map([[2, 4], [3, 5], [4, 6]]);
const credentialVersionKeys = new Set([
  'credential_version',
  'database_credential_version',
  'auth_credential_version',
  'credential_version_before',
  'credential_version_after',
  ...
  'from_credential_version',
  'to_credential_version',
]);

function adaptRun006Value(value: any, key = ''): any {
  if (Array.isArray(value)) return value.map(item => adaptRun006Value(item));
  if (value && typeof value === 'object') {
    const adapted = Object.fromEntries(
      Object.entries(value).map(([entryKey, entryValue]) => [
        entryKey,
        entryKey === 'context' ? entryValue : adaptRun006Value(entryValue, entryKey),
      ]),
    );
    if ('operatorConfirmationContract' in adapted) {
      adapted.operatorConfirmationContract = currentOperatorConfirmationContract();
    }
    if ('operator_confirmation' in adapted && adapted.operator_confirmation) {
      adapted.operator_confirmation.contract_id = CURRENT_OPERATOR_CONFIRMATION_CONTRACT_ID;
    }
    return adapted;
  }
  if (
    credentialVersionKeys.has(key)
    && typeof value === 'number'
    && historicalCredentialVersionMap.has(value)
  ) {
    return historicalCredentialVersionMap.get(value);
  }
  return value;
}

function historicalWrapper<T extends (...args: any[]) => any>(current: T): T {
  return ((...args: any[]) => current(...args.map(argument => adaptRun006Value(argument)))) as T;
}

const assertAfterPasswordChangeAggregate = historicalWrapper(assertCurrentAfterPasswordChangeAggregate);
const assertBeforeRequiredPasswordChangeAggregate = historicalWrapper(assertCurrentBeforeRequiredPasswordChangeAggregate);
const assertCompletedExecutionEvidence = historicalWrapper(assertCurrentCompletedExecutionEvidence);
const assertExactResetConfirmationsProof = historicalWrapper(assertCurrentExactResetConfirmationsProof);
const assertFinalFreshLoginAggregate = historicalWrapper(assertCurrentFinalFreshLoginAggregate);
const assertInitialEmployeeStateAggregate = historicalWrapper(assertCurrentInitialEmployeeStateAggregate);
const assertPasswordChangeCompletionProof = historicalWrapper(assertCurrentPasswordChangeCompletionProof);
const assertPostResetSuccessAggregate = historicalWrapper(assertCurrentPostResetSuccessAggregate);
const assertPreResetCheckpointAggregate = historicalWrapper(assertCurrentPreResetCheckpointAggregate);
const assertRuntimeAndContractsExactAggregate = historicalWrapper(assertCurrentRuntimeAndContractsExactAggregate);

function assertRun006ConfirmationAndFinalSessionContracts(input: any) {
  ...
  if (
    ...
    || confirmation?.run_number !== 6
    || confirmation?.exact_phrase !== 'EXECUTE RUN 006 RESET NOW'
    ...
    throw new Error('PATCH83U_RUN006_CONFIRMATION_AND_FINAL_SESSION_CONTRACTS_FAILED');
  }
  return input;
}
...
function auditSnapshot(
  ...
  return {
    ...
    latest_event_credential_version: passwordChange ? 4 : 3,
    ...
    latest_operation_current_version: passwordChange ? 3 : 2,
    latest_operation_next_version: passwordChange ? 4 : 3,
...
function resetEnvelope(requestId = 'patch83u-run006-request-001') {
  return {
...
describe('Patch 83U Run 006 completion evidence contract', () => {
  ...
  it('freeze-binds Run 006 confirmation and final 1/1 contracts together', () => {
    expect(assertRun006ConfirmationAndFinalSessionContracts({
      operatorConfirmationContract: run006OperatorConfirmationContract(),
    ...
    for (const drift of [
      ...
      expect(() => assertRun006ConfirmationAndFinalSessionContracts(
        ...
        /PATCH83U_RUN006_CONFIRMATION_AND_FINAL_SESSION_CONTRACTS_FAILED/,
      );
    }
  });
  ...
  it('proves pre-reset checkpoint combines v2 target, session minimum, admin, and runtime', () => {
  ...
  it('proves post-reset success combines v3 zero-session governance, audit, admin, runtime, and reconciliation', () => {
  ...
  it('proves before-change checkpoint combines v3 governance, exact temporary 1/1, admin, and runtime', () => {
    const referenceSnapshot = checkpointSnapshot('immediately_before_reset');
  ...
  it('proves after-change checkpoint combines v4 zero-session finalization, audit, admin, runtime, and signout', () => {
    const referenceSnapshot = checkpointSnapshot('immediately_before_reset');
  ...
  it('proves final checkpoint combines fresh authorization, active v4 exact 1/1, admin, and runtime', async () => {
    const context = await createRegisteredNonpersistentContext();
    const referenceSnapshot = checkpointSnapshot('immediately_before_reset');
  ...
  it('proves the complete initial Employee v2 governance state', () => {
  ...
  it('proves the reset request correlation is generated at submission and remains one stable hash', async () => {
    ...
    expect(() => premature.observeRequestId('patch83u-run006-request-001')).toThrow(
