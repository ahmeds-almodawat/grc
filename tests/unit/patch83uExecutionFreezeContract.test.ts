...
const freezeSchema = JSON.parse(
  readFileSync(
    resolve(
      'release/patch83u/'
        + 'patch83u-staging-reset-execution-freeze-schema-v7.json',
    ),
    'utf8',
  ),
);
...
const RUN_NUMBER = 7;
const RUN007_EVIDENCE_DIRECTORY = capturedFreeze.run_contract.evidence_directory;
const SQL_EDITOR_CHECKPOINT_DIRECTORY = capturedFreeze.run_contract.checkpoint_directory;
const RUN007_OUTPUT_PATH_PATTERN = capturedFreeze.run_contract.output_path_pattern;
const RUN007_EVIDENCE_SCHEMA_PATH = capturedFreeze.run_contract.evidence_schema_path;
const RESET_PROOF_SCHEMA_VERSION = capturedFreeze.run_contract.evidence_schema_version;
const RUN007_CHECKPOINT_SCHEMA_PATH = capturedFreeze.checkpoint_schema.path;
const RUN007_PROOF_CONTRACT_PATH = capturedFreeze.proof_contract.path;
const RUN007_PROOF_CONTRACT_VERSION = capturedFreeze.proof_contract.schema_version;
const RUN007_PROOF_CONTRACT_ID = capturedFreeze.proof_contract.contract_id;
const RUN007_TRACEABILITY_PATH = capturedFreeze.traceability.json_path;
const RUN007_TRACEABILITY_MARKDOWN_PATH = capturedFreeze.traceability.markdown_path;
const RUN007_TRACEABILITY_VERSION = capturedFreeze.traceability.schema_version;
const RUN007_OPERATOR_CONFIRMATION_CONTRACT_ID =
...
function validFreezeFixture() {
  ...
  const evidenceSchema = localFileManifest(RUN007_EVIDENCE_SCHEMA_PATH);
  const proofContract = localFileManifest(RUN007_PROOF_CONTRACT_PATH);
  const checkpointSchema = localFileManifest(RUN007_CHECKPOINT_SCHEMA_PATH);
  const traceabilityJson = localFileManifest(RUN007_TRACEABILITY_PATH);
  const traceabilityMarkdown = localFileManifest(
    RUN007_TRACEABILITY_MARKDOWN_PATH,
  );
  const proofDocument = JSON.parse(
    readFileSync(resolve(RUN007_PROOF_CONTRACT_PATH), 'utf8'),
  );
  ...
  return {
    ...
    supersedes: {
      path:
        'release/patch83u/'
        + 'patch83u-staging-reset-execution-freeze-v6-20260720.json',
      sha256:
...
describe('Patch 83U V7 execution-freeze contract', () => {
  ...
  it('cryptographically binds the Run 007 proof and traceability artifacts', async () => {
    const hostedCall = vi.fn();
    vi.stubGlobal('fetch', hostedCall);
    const fixture = validFreezeFixture();

    await expect(verifyRun007ContractArtifacts({
      freeze: fixture,
    ...
    await expect(verifyRun007ContractArtifacts({
      freeze: drifted,
      repositoryRoot: process.cwd(),
    })).rejects.toThrow('PATCH83U_RUN007_PROOF_CONTRACT_HASH_MISMATCH');
    expect(hostedCall).not.toHaveBeenCalled();
  });

  it('rejects any incomplete Run 007 traceability row', () => {
    const proofContract = JSON.parse(
      readFileSync(resolve(RUN007_PROOF_CONTRACT_PATH), 'utf8'),
    );
    const traceability = JSON.parse(
      readFileSync(resolve(RUN007_TRACEABILITY_PATH), 'utf8'),
    );
    expect(validateRun007ProofContract(proofContract)).toMatchObject({
      passed: true,
      phase_count: 14,
      requirement_count: 56,
    });
    expect(validateRun007Traceability(
      traceability,
      proofContract,
    )).toMatchObject({
      passed: true,
    ...
    expect(() => validateRun007Traceability(
      incomplete,
      proofContract,
    )).toThrow('PATCH83U_RUN007_TRACEABILITY_TEST_MISSING');
  });
  ...
  it('rejects Run 007 confirmation and final 1/1 count drift without hosted calls', () => {
    const hostedCall = vi.fn(() => {
  ...
  it('rejects incomplete or partially mapped Run 007 traceability', () => {
    for (const [pointer, value] of [
