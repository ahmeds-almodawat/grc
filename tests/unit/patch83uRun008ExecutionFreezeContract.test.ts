...
import {
  ...
  EXECUTION_FREEZE_CONSUMED_JSON_POINTERS as CURRENT_FREEZE_POINTERS,
...
import {
  createHistoricalFreezeAssertions,
  historicalPointersForSchema,
} from './helpers/patch83uHistoricalFreezeContract';
import {
  validateRun008ProofContract,
  validateRun008Traceability,
  verifyRun008ContractArtifacts,
} from '../../scripts/patch83u-run008-contract-audit.mjs';

const freezeSchema = JSON.parse(
  readFileSync(
    resolve(
      'release/patch83u/'
        + 'patch83u-staging-reset-execution-freeze-schema-v8.json',
    ),
    'utf8',
  ),
);
const capturedFreeze = JSON.parse(readFileSync(resolve(
  'release/patch83u/patch83u-staging-reset-execution-freeze-v8-20260721.json',
), 'utf8'));
const EXECUTION_FREEZE_CONSUMED_JSON_POINTERS = historicalPointersForSchema(
  CURRENT_FREEZE_POINTERS,
  freezeSchema,
);
const EXECUTION_FREEZE_READY_DECISION = capturedFreeze.readiness_decision;
const EXECUTION_FREEZE_SCHEMA_VERSION = capturedFreeze.schema_version;
const RUN_NUMBER = 8;
const RUN008_EVIDENCE_DIRECTORY = capturedFreeze.run_contract.evidence_directory;
const SQL_EDITOR_CHECKPOINT_DIRECTORY = capturedFreeze.run_contract.checkpoint_directory;
const RUN008_OUTPUT_PATH_PATTERN = capturedFreeze.run_contract.output_path_pattern;
const RUN008_EVIDENCE_SCHEMA_PATH = capturedFreeze.run_contract.evidence_schema_path;
const RESET_PROOF_SCHEMA_VERSION = capturedFreeze.run_contract.evidence_schema_version;
const RUN008_CHECKPOINT_SCHEMA_PATH = capturedFreeze.checkpoint_schema.path;
const RUN008_PROOF_CONTRACT_PATH = capturedFreeze.proof_contract.path;
const RUN008_PROOF_CONTRACT_VERSION = capturedFreeze.proof_contract.schema_version;
const RUN008_PROOF_CONTRACT_ID = capturedFreeze.proof_contract.contract_id;
const RUN008_TRACEABILITY_PATH = capturedFreeze.traceability.json_path;
const RUN008_TRACEABILITY_MARKDOWN_PATH = capturedFreeze.traceability.markdown_path;
const RUN008_TRACEABILITY_VERSION = capturedFreeze.traceability.schema_version;
const RUN008_OPERATOR_CONFIRMATION_CONTRACT_ID =
  capturedFreeze.operator_confirmation_contract.contract_id;
const EXPECTED_FINAL_SESSION_COUNT =
  capturedFreeze.final_session_contract.expected_session_count;
const EXPECTED_FINAL_UNREVOKED_REFRESH_TOKEN_COUNT =
  capturedFreeze.final_session_contract.expected_unrevoked_refresh_token_count;
const {
  assertExecutionFreezeConsumedFields,
  assertExecutionFreezeSemanticContract,
} = createHistoricalFreezeAssertions({
  capturedFreeze,
  consumedPointers: EXECUTION_FREEZE_CONSUMED_JSON_POINTERS,
  runNumber: 8,
});
...
function validFreezeFixture() {
  ...
  const evidenceSchema = localFileManifest(RUN008_EVIDENCE_SCHEMA_PATH);
  const proofContract = localFileManifest(RUN008_PROOF_CONTRACT_PATH);
  const checkpointSchema = localFileManifest(RUN008_CHECKPOINT_SCHEMA_PATH);
  const traceabilityJson = localFileManifest(RUN008_TRACEABILITY_PATH);
  const traceabilityMarkdown = localFileManifest(
    RUN008_TRACEABILITY_MARKDOWN_PATH,
  );
  const proofDocument = JSON.parse(
    readFileSync(resolve(RUN008_PROOF_CONTRACT_PATH), 'utf8'),
  );
  ...
  return {
    $schema: './patch83u-staging-reset-execution-freeze-schema-v8.json',
    ...
    supersedes: {
      path:
        'release/patch83u/'
        + 'patch83u-staging-reset-execution-freeze-v7-20260721.json',
      sha256:
    ...
    operator_confirmation_contract: {
      contract_id: RUN008_OPERATOR_CONFIRMATION_CONTRACT_ID,
      run_number: RUN_NUMBER,
      exact_phrase: 'EXECUTE RUN 008 RESET NOW',
      case_sensitive: true,
...
describe('Patch 83U V8 execution-freeze contract', () => {
  ...
  it('cryptographically binds the Run 008 proof and traceability artifacts', async () => {
    const hostedCall = vi.fn();
    vi.stubGlobal('fetch', hostedCall);
    const fixture = validFreezeFixture();

    await expect(verifyRun008ContractArtifacts({
      freeze: fixture,
    ...
    await expect(verifyRun008ContractArtifacts({
      freeze: drifted,
      repositoryRoot: process.cwd(),
    })).rejects.toThrow('PATCH83U_RUN008_PROOF_CONTRACT_HASH_MISMATCH');
    expect(hostedCall).not.toHaveBeenCalled();
  });

  it('rejects any incomplete Run 008 traceability row', () => {
    const proofContract = JSON.parse(
      readFileSync(resolve(RUN008_PROOF_CONTRACT_PATH), 'utf8'),
    );
    const traceability = JSON.parse(
      readFileSync(resolve(RUN008_TRACEABILITY_PATH), 'utf8'),
    );
    expect(validateRun008ProofContract(proofContract)).toMatchObject({
      passed: true,
      phase_count: 14,
      requirement_count: 56,
    });
    expect(validateRun008Traceability(
      traceability,
      proofContract,
    )).toMatchObject({
      passed: true,
    ...
    expect(() => validateRun008Traceability(
      incomplete,
      proofContract,
    )).toThrow('PATCH83U_RUN008_TRACEABILITY_TEST_MISSING');
  });
  ...
  it('rejects Run 008 confirmation and final 1/1 count drift without hosted calls', () => {
    const hostedCall = vi.fn(() => {
  ...
  it('rejects incomplete or partially mapped Run 008 traceability', () => {
    for (const [pointer, value] of [
