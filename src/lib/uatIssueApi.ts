...
export async function createUatIssue(input: UatIssueInput) {
  ...
  const payload = {
    ...
    screenshot_note: input.screenshotNote?.trim() || null,
  ...
  const { data, error } = await supabase
    ...
    .insert(payload)
    .select('id,title,module,role_account_used,steps_to_reproduce,expected_result,actual_result,screenshot_note,severity,status,created_at')
    .single();
