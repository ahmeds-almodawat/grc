...
export function EvidenceCenter({ setPage }: EvidenceCenterProps) {
  ...
  const repository = <div className="ui6-evidence-table" role="table" aria-label={text('Evidence repository', 'مستودع الأدلة')}>
    <div className="ui6-evidence-row ui6-evidence-row--head" role="row" key="evidence-header"><span>{text('Evidence', 'الدليل')}</span><span>{text('Type', 'النوع')}</span><span>{text('Source / Usage', 'المصدر والاستخدام')}</span><span>{text('Owner', 'المالك')}</span><span>{text('Status', 'الحالة')}</span><span>{text('Uploaded', 'تاريخ الرفع')}</span></div>
