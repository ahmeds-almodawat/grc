...
export function Compliance() {
  ...
  if (selected) {
    ...
    return (
      <section className="page-section ui3-module" data-testid="ui3-compliance-detail">
        ...
        {tab === 'governance' ? selectedAssessment ? <GovernanceCriteriaLinkage source={{ type: 'compliance_assessment', id: selectedAssessment.id, organizationId: selectedAssessment.organization_id || selected.organization_id || organizationId, sourceDate: selectedAssessment.assessment_date, departmentId: selectedAssessment.department_id }} mode="compliance" title={text('Obligation and Internal Governance Basis', 'الالتزام وأساس الحوكمة الداخلية')} canManage={canManage} requiredObligationId={selected.id} onReviewChange={setGovernanceReview} /> : <section className="ui3-surface"><EmptyState label={text('Select or create an assessment to establish its exact governance basis', 'اختر أو أنشئ تقييماً لتحديد أساس الحوكمة الدقيق')} /></section> : null}
  ...
  const openFindings = rows.reduce((total, item) => total + item.open_finding_count, 0);
  ...
  return (
    <section className="page-section ui3-module" data-testid="ui3-compliance-register">
      ...
      <div className="ui3-view-switch" role="tablist"><button type="button" className={view === 'register' ? 'active' : ''} onClick={() => setView('register')}><List size={16} />{text('Obligations', 'الالتزامات')}</button><button type="button" className={view === 'dashboard' ? 'active' : ''} onClick={() => setView('dashboard')}><LayoutDashboard size={16} />{text('Dashboard', 'لوحة المعلومات')}</button></div>
      {notice ? <div className="ui3-notice" role="status">{notice}</div> : null}
