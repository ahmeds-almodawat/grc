...
export function Audit() {
  ...
  return (
    <section className="page-section ui3-module ui4-module" data-testid="ui4-audit">
      ...
      <nav className="ui4-workspace-tabs" aria-label={text('Audit workspace views', 'عروض مساحة عمل المراجعة')}>{auditScreens.map((item) => { const Icon = item.icon; const unavailable = ['engagement','planning','program','report'].includes(item.id) && !selectedEngagement; return <button type="button" key={item.id} aria-label={item.label} className={screen === item.id ? 'active' : ''} disabled={unavailable || (item.id === 'finding' && !selectedFinding)} onClick={() => setScreen(item.id)}><Icon size={15} /><span>{item.label}</span></button>; })}</nav>
