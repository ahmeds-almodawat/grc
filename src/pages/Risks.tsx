...
export function Risks() {
  ...
  return (
    <section className="page-section ui3-module" data-testid="ui3-risk-register">
      ...
      <div className="ui3-view-switch" role="tablist"><button type="button" className={view === 'register' ? 'active' : ''} onClick={() => setView('register')}><List size={16} />{text('Risk register', 'سجل المخاطر')}</button><button type="button" className={view === 'dashboard' ? 'active' : ''} onClick={() => setView('dashboard')}><LayoutDashboard size={16} />{text('Dashboard', 'لوحة المعلومات')}</button></div>
      {notice ? <div className="ui3-notice" role="status">{notice}</div> : null}
