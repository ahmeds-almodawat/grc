...
export function OperationsCenter() {
  ...
  const [selectedItem, setSelectedItem] =
    useState<SelectedOperationsItem | null>(null);
  ...
  useEffect(() => {
    const ctxString = sessionStorage.getItem("grc.departmentContext");
    if (ctxString) {
      try {
        const ctx = JSON.parse(ctxString);
        setDepartmentContext(ctx);
        setSearch(ctx.departmentName);
      } catch (e) {
        console.error("Invalid department context", e);
      }
    }
  ...
  const filteredReminders = useMemo(() => {
    ...
    return state.reminders.filter((item) => {
      ...
      const matchesSearch =
        !q ||
        [item.title, item.item_type, item.reminder_type, item.status].some(
  ...
  return (
    <div className="page-section operations-page">
      ...
      <div className="stats-grid operations-kpi-grid">
        <button
          type="button"
          className={`stat-card warning ${activeFilter === "unread" ? "active" : ""}`}
          onClick={() => focusOperationsFilter("unread", "digest")}
        >
          <BellRing size={21} />
        ...
        <button
          type="button"
          className={`stat-card danger ${activeFilter === "overdue" ? "active" : ""}`}
          onClick={() => focusOperationsFilter("overdue", "inbox")}
        >
        ...
        <button
          type="button"
          className={`stat-card warning ${activeFilter === "approvals" ? "active" : ""}`}
          onClick={() => focusOperationsFilter("approvals", "inbox")}
        >
        ...
        <button
          type="button"
          className={`stat-card ${activeFilter === "evidence" ? "active" : ""}`}
          onClick={() => focusOperationsFilter("evidence", "inbox")}
        >
          <Inbox size={21} />
          <div className="stat-value">
            {summary?.pending_evidence_reviews ?? "—"}
          </div>
          <div className="stat-label">{t("ops.pendingEvidence")}</div>
        </button>
        <button
          type="button"
          className={`stat-card danger ${activeFilter === "escalations" ? "active" : ""}`}
          onClick={() => focusOperationsFilter("escalations", "inbox")}
        >
          <Clock3 size={21} />
        ...
        <button
          type="button"
          className={`stat-card ${activeFilter === "ovr" ? "active" : ""}`}
          onClick={() => focusOperationsFilter("ovr", "inbox")}
        >
          <UsersRound size={21} />
      ...
      <div className="panel operations-control-panel">
        <div className="split-header">
          ...
          <div className="toolbar">
            <span className="status-badge status-info">
              Active filter: {labelize(activeFilter)}
            </span>
            <input
              value={search}
        ...
        <div className="segmented-tabs operations-tabs">
          {(["inbox", "reminders", "digest", "timeline"] as TabKey[]).map(
            (key) => (
              <button
                key={key}
              ...
              <tbody>
                {filteredInbox.map((item) => (
                  <tr key={`${item.item_type}-${item.item_id}`}>
                      ...
                      <button
                        ...
                        onClick={() =>
                          setSelectedItem({
                            ...
                            owner:
                              language === "ar"
                                ? (item.owner_name_ar ?? item.owner_name_en)
                                : (item.owner_name_en ?? item.owner_name_ar),
                            department:
                              language === "ar"
                                ? (item.department_name_ar ??
                                  item.department_name_en)
                            ...
                            nextAction:
                              "Review the source item and follow the assigned manager action.",
                          })
                        }
                      >
                    ...
                    <td>
                      <span
                        className={`status-badge status-${toneForReminder(item.reminder_type, item.risk_level)}`}
                      >
                        {labelize(item.reminder_type)}
                      </span>
                    </td>
                    <td>
                      <span
                        className={`risk-pill ${item.risk_level ?? "medium"}`}
                      >
                        {labelize(item.risk_level)}
                      </span>
                    </td>
        ...
        {tab === "reminders" && (
          <div className="operations-card-grid">
            {filteredReminders.map((item) => (
              <button
                type="button"
                className={`work-card reminder-card ${toneForReminder(item.reminder_type, item.risk_level)}`}
                  ...
                  setSelectedItem({
                    ...
                    nextAction:
                      item.days_overdue > 0
                        ...
                        : "Owner follow-up should be completed before the due date.",
                  })
                }
              >
                <div className="work-card-header">
                  ...
                  <span className={`risk-pill ${item.risk_level ?? "medium"}`}>
                    {labelize(item.risk_level)}
                  </span>
                </div>
                <h5>{item.title}</h5>
                <p>
                  {labelize(item.item_type)} · {labelize(item.status)} ·{" "}
                  {formatDate(item.due_date)}
                </p>
                {item.days_overdue > 0 && (
                  <strong className="danger-text">
                    {item.days_overdue} {t("ops.daysOverdue")}
                  </strong>
                )}
              </button>
            ))}
                ...
                {state.digest.map((row) => (
                  <tr key={row.user_id}>
                    <td>
                      <strong>
                        {language === "ar"
                          ? (row.full_name_ar ?? row.full_name_en)
                          : (row.full_name_en ?? row.full_name_ar)}
                      </strong>
          ...
          <div className="activity-timeline">
            {state.timeline.map((event, index) => (
              <div
                ...
                key={`${event.event_source}-${event.item_id ?? index}-${event.created_at}`}
              >
                ...
                <div>
                  ...
                  <p>
                    {language === "ar"
                      ? (event.actor_name_ar ?? event.actor_name_en)
                      : (event.actor_name_en ?? event.actor_name_ar)}{" "}
                    · {labelize(event.event_source)} ·{" "}
        ...
        {selectedItem ? (
          <div className="detail-panel">
            ...
            <div className="detail-grid">
              <div>
                <span>Type</span>
                <strong>{labelize(selectedItem.type)}</strong>
              </div>
              <div>
                <span>Status</span>
                <strong>{labelize(selectedItem.status)}</strong>
              </div>
              <div>
                <span>Owner</span>
                <strong>{selectedItem.owner || "Unassigned"}</strong>
              </div>
              <div>
                <span>Department</span>
                <strong>{selectedItem.department || "Company-wide"}</strong>
              </div>
              <div>
                <span>Due date</span>
                <strong>{formatDate(selectedItem.due)}</strong>
              </div>
              <div>
                <span>Risk</span>
                <strong>{labelize(selectedItem.risk)}</strong>
              </div>
              <div>
