...
select lives_ok($$select public.f1r2_create_work_item(pg_temp.f2_uuid(11),'milestone',jsonb_build_object('project_id',(select id from public.projects where title='Pending privacy project'),'title','Pending privacy milestone','owner_id',pg_temp.f2_uuid(17),'start_date','2026-08-16','due_date','2026-08-22'))$$,'pending milestone fixture is created by independent project creator');
...
select lives_ok($$select public.f1r2_create_work_item(pg_temp.f2_uuid(11),'milestone',jsonb_build_object('project_id',(select id from public.projects where title='Terminal container project'),'title','Closed assignment milestone','owner_id',pg_temp.f2_uuid(17),'start_date','2026-08-16','due_date','2026-08-22'))$$,'closed milestone begins pending');
select lives_ok($$select public.f1r2_create_work_item(pg_temp.f2_uuid(11),'milestone',jsonb_build_object('project_id',(select id from public.projects where title='Terminal container project'),'title','Cancelled assignment milestone','owner_id',pg_temp.f2_uuid(17),'start_date','2026-08-16','due_date','2026-08-22'))$$,'cancelled milestone begins pending');
select lives_ok($$select public.f1r2_create_work_item(pg_temp.f2_uuid(11),'milestone',jsonb_build_object('project_id',(select id from public.projects where title='Terminal container project'),'title','Terminal task container','start_date','2026-08-16','due_date','2026-08-22'))$$,'terminal task container milestone is created');
set local session_replication_role=replica;
...
select throws_ok($$select public.f1r2_create_work_item(pg_temp.f2_uuid(12),'milestone',jsonb_build_object('project_id',(select id from public.projects where title='F2 Corrective project'),'title','Missing dates'))$$,'F1R2_MILESTONE_DATES_REQUIRED','missing milestone dates fail closed');
select throws_ok($$select public.f1r2_create_work_item(pg_temp.f2_uuid(12),'milestone',jsonb_build_object('project_id',(select id from public.projects where title='F2 Corrective project'),'title','Partial dates','start_date','2026-08-16'))$$,'F1R2_MILESTONE_DATES_REQUIRED','partial milestone dates fail closed');
...
select is((select new_data->>'start_date' from public.audit_logs where action='f1r2_milestone_created' and record_id=(select id from public.milestones where title='M1')),'2026-08-15','milestone creation audit records the exact start date');
select is((select new_data->>'due_date' from public.audit_logs where action='f1r2_milestone_created' and record_id=(select id from public.milestones where title='M1')),'2026-08-25','milestone creation audit records the exact due date');
...
select lives_ok($$select public.f1r2_create_work_item(pg_temp.f2_uuid(11),'milestone',jsonb_build_object('project_id',(select id from public.projects where title='Task contract project'),'title','Task contract milestone','start_date','2026-08-16','due_date','2026-08-22'))$$,'task contract fixture milestone is created');
...
select lives_ok($$select public.f1r2_create_work_item(pg_temp.f2_uuid(11),'milestone',jsonb_build_object('project_id',(select id from public.projects where title='Relationship project B'),'title','Milestone B','start_date','2026-08-16','due_date','2026-08-22'))$$,'relationship fixture milestone B is created');
