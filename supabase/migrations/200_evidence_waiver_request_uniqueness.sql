-- =========================================================
-- Migration 200: Evidence Gate Waiver Request Uniqueness Invariant
-- Enforces at most one active requested waiver per evidence requirement
-- via a partial unique index and race-safe RPC concurrency handling.
-- =========================================================

-- 1. Create partial unique index on active requested waivers
create unique index if not exists uq_evidence_gate_waivers_active_req
on public.evidence_gate_waivers (requirement_id)
where status = 'requested';

comment on index public.uq_evidence_gate_waivers_active_req is
  'Migration 200: partial unique index guaranteeing at most one pending requested evidence gate waiver per requirement.';

-- 2. Update Patch 23 Evidence Governance Bridge RPC with concurrency-safe duplicate prevention
  ...
  if v_action = 'create_evidence_requirement' then
    ...
    if v_action = 'link_evidence_to_item' then
      ...
      if exists (
        ...
        raise exception 'PATCH23_EVIDENCE_WAIVER_ALREADY_REQUESTED';
      end if;

      -- Concurrency-safe insert protected by unique index
      begin
...
-- 3. Update Patch 43 compatibility wrapper function
...
begin
  ...
  if exists (
    select 1 from public.evidence_gate_waivers
    where requirement_id = v_requirement_id
      and status = 'requested'
  ) then
    raise exception 'PATCH23_EVIDENCE_WAIVER_ALREADY_REQUESTED';
