-- P3 correction cycle 3: carry the canonical division reference through the
-- required-password finalizer and its exact fail-closed recovery path.
--
-- Migration 223 corrected initial provisioning, but migration 189 still matched
-- the reserved role as if every non-department scope had no division reference.
-- That left a valid division-scoped account invited and recovery-required after
-- its successful first password change. The replacement functions below retain
-- the existing proofs while using the scope-qualified role reference throughout.
...
begin
  ...
  then
    raise exception 'PATCH83U_224_REQUIRED_FUNCTION_MISSING';
  end if;
end;
$patch83u_224_preflight$;
...
declare
  ...
  v_credential_update_count integer := 0;
  v_post_password_activation_recovered boolean := false;
  v_division_password_activation_recovery boolean := false;
  v_now timestamptz := clock_timestamp();
begin
  ...
  if v_employee_id_conflict then
    ...
    if v_profile.id is null then
      ...
      or v_profile.division_id is distinct from v_queue.division_id
    ...
    then
      ...
      v_division_password_activation_recovery :=
        v_queue.provisioning_status = 'reconciliation_required'
        and v_queue.last_error_code = 'PATCH83U_PROVISIONED_ROLE_ACTIVATION_FAILED'
        and v_queue.requested_scope = 'division'
        and v_state.credential_state = 'recovery_required'
        and v_state.operation_source = 'password_change'
      ...
      -- Preserve migration 189's narrow Employee recovery and add one exact
      -- division-scoped recovery for the fail-closed state produced by the old
      -- password finalizer. Both paths require the same immutable identity,
      -- Auth-version, zero-active-role, and exact reserved-role proofs below.
      elsif (
        v_queue.provisioning_status = 'initial_change_required'
        and v_state.credential_state = 'active'
        and v_profile.user_status = 'invited'
      ) or v_division_password_activation_recovery
      then
        perform ai.id
        ...
        if v_queue.auth_user_id is distinct from v_auth_user_id
          ...
          or (
            not v_division_password_activation_recovery
            and (
          ...
          or (
            v_division_password_activation_recovery
            and (
              v_queue.requested_scope is distinct from 'division'
              or v_queue.last_error_code is distinct from
                'PATCH83U_PROVISIONED_ROLE_ACTIVATION_FAILED'
              or v_queue.completed_at is not null
            )
          )
          ...
          or (
            not v_division_password_activation_recovery
            and (
              v_state.credential_state is distinct from 'active'
          ...
          or (
            v_division_password_activation_recovery
            and (
              v_state.credential_state is distinct from 'recovery_required'
        ...
        select v_org_id, v_auth_user_id, v_role_id, 'reactivated',
          to_jsonb(ur),
          case when v_division_password_activation_recovery
            then 'Patch 83U division password-change role activation recovery'
            else 'Patch 83U post-provisioning role activation recovery'
          end,
          p_actor_id
        from public.user_roles ur
        where ur.id = v_role_id;
        ...
        if v_division_password_activation_recovery then
          update public.user_credential_states
          ...
          get diagnostics v_credential_update_count = row_count;
          if v_credential_update_count <> 1 then
            raise exception 'PATCH83U_POST_PROVISIONING_CREDENTIAL_ACTIVATION_CHANGED';
          end if;
        end if;
  ...
  insert into public.user_credential_events (
    ...
    case
      when v_division_password_activation_recovery
        then 'PATCH83U_DIVISION_PASSWORD_ROLE_ACTIVATED'
    ...
    jsonb_build_object(
      ...
      'division_password_activation_recovered',
        v_division_password_activation_recovery,
      'role_rows_changed', v_role_update_count,
      'profile_status_rows_changed', v_profile_update_count,
      'credential_state_rows_changed', v_credential_update_count,
      'credential_state_preserved',
        v_post_password_activation_recovered
        and not v_division_password_activation_recovery,
...
'Service-only finalization with exact operation/request/Auth proof. Migration 224 preserves the canonical division or department reference when activating the one reserved provisioning role.';

comment on function public.patch83u_reconcile_provisioning(
  uuid, uuid, text, text
) is
'Service-only, Super-Admin and exact-Employee-ID reconciliation. Migration 224 retains migration 189 recovery and adds one fail-closed division password-activation recovery when Auth, profile, credential version, provisioning identity, zero-active-role, and exact inactive-role proofs all agree.';
